...
output_file_path = 'tree_numbers_counter_uniform.pkl'
...
plt.xlabel('log(x)')
plt.ylabel('log(f(x))')
plt.title('График f(x) - количество деревьев, которые потенциально строят лемму по уникальной словоформе только x раз')

plt.legend()

# Показ графика
plt.savefig('график_как_из_статьи_2.png')  # Сохраняем график
